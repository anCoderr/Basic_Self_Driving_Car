...
# BRO! Using this you can configure the size of our canvas.
...
# Our AI, that contains our neural network and represents our Q-function
brain = Dqn(5, 3, 0.9)  # 5 sensors, 3 actions, gama = 0.9
action2rotation = [0, 20, -20]  # action = 0 => no rotation, action = 1 => rotate 20 degrees, action = 2 => rotate -20 degres
...
def init():
    ...
    global first_update
    sand = np.zeros((canvas_length, canvas_width))  # initializing the sand array with only zeros
    goal_x = 20  # the goal is to reach at the upper left of the map (the x-coordinate is 20 and not 0 because the car gets bad reward if it touches the wall)
    goal_y = canvas_width - 20  # the goal to reach is at the upper left of the map (y-coordinate)
...
class Car(Widget):
    angle = NumericProperty(0)  # angle of the car (angle between the x-axis of the map and the axis of the car)
    rotation = NumericProperty(0)  # last rotation of the car (after playing the action, the car rotates by 0, 20 or -20 degrees)
    ...
    def move(self, rotation):
        ...
        if self.sensor1_x > canvas_length - 10 or self.sensor1_x < 10 or self.sensor1_y > canvas_width - 10 or self.sensor1_y < 10:  # if sensor 1 is out of the map (the car is facing one edge of the map)
            self.signal1 = 1.  # sensor 1 detects full sand
        if self.sensor2_x > canvas_length - 10 or self.sensor2_x < 10 or self.sensor2_y > canvas_width - 10 or self.sensor2_y < 10:  # if sensor 2 is out of the map (the car is facing one edge of the map)
            self.signal2 = 1.  # sensor 2 detects full sand
        if self.sensor3_x > canvas_length - 10 or self.sensor3_x < 10 or self.sensor3_y > canvas_width - 10 or self.sensor3_y < 10:  # if sensor 3 is out of the map (the car is facing one edge of the map)
...
class Game(Widget):
    ...
    def update(self, df):  # the big update function that updates everything that needs to be updated at each discrete time t when reaching a new state (getting new signals from the sensors)
        ...
        global canvas_length  # specifying the global variables (width of the map)
        global canvas_width  # specifying the global variables (height of the map)

        canvas_length = self.width  # width of the map (horizontal edge)
        canvas_width = self.height  # height of the map (vertical edge)
        ...
        if sand[int(self.car.x), int(self.car.y)] > 0:  # if the car is on the sand
            ...
            if distance < last_distance:  # however if it getting close to the goal
                last_reward = 0.2 # it still gets slightly positive reward 0.2
        ...
        # Update the last distance from the car to the goal
